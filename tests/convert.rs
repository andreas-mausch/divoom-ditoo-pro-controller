...
fn convert_gif_to_divoom16() -> Result<(), Box<dyn Error>> {
  ...
  asserting("Divoom16 delay").that(&output[3]).is_equal_to(120);
  asserting("Divoom16 delay").that(&output[130]).is_equal_to(120);
  asserting("Divoom16 delay").that(&output[233]).is_equal_to(120);
  asserting("Divoom16 delay").that(&output[336]).is_equal_to(120);
  ...
  asserting("Divoom16 output").that(&output).is_equal_to(&expected_output);
