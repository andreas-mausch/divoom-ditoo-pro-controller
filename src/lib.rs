...
fn create_network_packets_from(animation: &[u8]) -> Result<Vec<Vec<u8>>, Box<dyn Error>> {
  ...
  let mut xxx = animation.chunks(256).enumerate().map(|(index, chunk)| {
...
pub fn send_divoom_animation<R: Read>(mac_address: BtAddr, reader: &mut R) -> Result<(), Box<dyn Error>> {
  let mut animation = Vec::new();
  reader.read_to_end(&mut animation)?;
  info!("{:?}", animation);
  let packets = create_network_packets_from(&animation)?;
  send(mac_address, &packets.iter().map(|packet| packet.as_slice()).collect::<Vec<&[u8]>>())?;
  Ok(())
}
